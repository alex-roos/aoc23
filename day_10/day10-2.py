...
def inside_shape(loop_artifacts):
    # psuedo-code: parse the list of pipe tiles with state to capture the number of actual pipe edges

    pass 

if DEBUG:
    #file = open("C:\\Users\\alexr\\git\\aoc23\\day_10\\day_10_input_test.txt", "r")
    file = open("C:\\Users\\alexr\\dev\\aoc23\\day_10\\day_10_input_test_2.txt", "r")
else:
...
COUNT_COLS = len(original_data[0])
...
while (not left_terminated or not right_terminated):
    ...
    for _curr_traverser in traversers:
        ...
        final_paths[_curr_traverser.current_node_loc[0]][_curr_traverser.current_node_loc[1]] = PIPE_DICT[_curr_traverser.current_node_loc].symbol
            ...
            for j in range(i, len(row)):
                if state == 'F':
                    ...
                    if row[j] == '7':
                        ...
                        state = ''
                        count_walls += 1
                elif row[j] in ['S', '7', 'F', 'J', 'L', '|']:
                    state = row[j]

                    count_walls += 1

            if count_walls > 0 and count_walls % 2 == 1:
                in_loop_count += 1
                final_paths[row_id][i] = '+'

    row_id += 1

print("Count tiles in loop:", in_loop_count)
